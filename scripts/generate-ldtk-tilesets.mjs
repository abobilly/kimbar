 ...
 * 
 * Reads: specs/ai_jobs/tileset_manifest.json
 * Outputs: 
 *   - public/generated/tilesets/_scotus_tileset_def.json (tileset definition)
 *   - public/generated/tilesets/_tile_mapping.json (tile id mapping)
 * 
 * Usage: 
...
// Paths
const MANIFEST_PATH = path.join(__dirname, '../specs/ai_jobs/tileset_manifest.json');
const ROOM_REQUIREMENTS_PATH = path.join(__dirname, '../specs/ai_jobs/room_tile_requirements.json');
const GENERATED_TILES_DIR = path.join(__dirname, '../public/generated/tiles');
const PUBLIC_TILES_DIR = path.join(__dirname, '../public/generated/tiles');
const LDTK_OUTPUT_DIR = path.join(__dirname, '../public/generated/tilesets');
...
function buildLdtkTilesetDef(manifest, generatedTiles) {
  ...
  const tilesetDef = {
    ...
    relPath: "../tiles/_combined_tileset.png",
    embedAtlas: null,
    pxWid: width,
    pxHei: height,
...
function buildCategoryTilesets(manifest, generatedTiles) {
  ...
  for (const [category, tiles] of Object.entries(groups)) {
    ...
    const tilesetDef = {
      ...
      relPath: `../tiles/_${category}_tileset.png`,
      pxWid: width,
      pxHei: height,
      tileGridSize: TILE_SIZE,
      spacing: 0,
